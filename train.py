...
def train_epoch(model, loader, optimizer, criterion, scaler, device):
    ...
    # Determine device type for autocast (if needed)
    ...
    for images, masks in pbar:
        ...
        # --- Mixed Precision Path (CUDA / supported Hardware) ---
        ...
        # --- Standard Path (Mac MPS / CPU) ---
        else:
            # MPS often runs better in standard float32 for stability
            outputs = model(images)
            loss = criterion(outputs, masks)
            loss.backward()
            optimizer.step()
...
def main():
    ...
    # Scaler Logic: Only for CUDA currently in this setup
    if Config.DEVICE == 'cuda':
        scaler = torch.amp.GradScaler('cuda')
    else:
        scaler = None  # Disable scaler for MPS/CPU to prevent errors
